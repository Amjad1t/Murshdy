...
if ($conn->connect_error) {
 ...
 if (isset($_SESSION['user_id'])) {
    ...
    if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
        ...
        $sql = "INSERT INTO scheduled (S_ID, file_name,Notes, Date, Status,A_ID) VALUES ('$student_id', '$target_file','$notes', '$upload_date', '$status','$A_id')";

        if ($conn->query($sql) === TRUE) {
           
        header("Location: Stu_Schedule.php?error=0");
        } else {
           header("Location: Stu_Schedule.php?error=1");
        }
    }
}
 }
