...
class StudentChat { 
    ...
    private $advisorTable = 'advisor';
    private $studentTable = 'student';	// Assuming you have a table for advisors
    ...
    public function chatUsers($student_id){
		$sqlQuery = "
		   SELECT * FROM ".$this->advisorTable." 
			WHERE A_ID = (
				SELECT A_ID FROM ".$this->studentTable." 
				WHERE username = '$student_id'
			)";
		return $this->getData($sqlQuery);
   }

    public function getUserDetails($advisorId){
        $sqlQuery = "
            SELECT * FROM ".$this->advisorTable." 
