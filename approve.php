...
if ($result_advisor_info->num_rows > 0) {
    ...
    if (isset($_GET['id']) && isset($_GET['si_id'])) {
        $student_id = $_GET['id'];
        $sick_leave_id = $_GET['si_id'];
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $sql_update_status = "UPDATE sick_leave SET status = '$new_status' WHERE S_ID = '$student_id' AND Si_ID = '$sick_leave_id' AND A_ID = '$advisor_id'";
            if ($conn->query($sql_update_status) === TRUE) {
                header("Location: Adv_Sickleaves.php?error=0");
                ...
                header("Location: Adv_Sickleaves.php?error=1");
                // Add JavaScript to trigger an alert after redirect
                echo "<script>";
        ...
        $sql_sick_leave_info = "SELECT student.Name AS student_name, sick_leave.Date, sick_leave.Status, sick_leave.file_path
                                FROM sick_leave
                                JOIN student ON sick_leave.S_ID = student.S_ID
                                WHERE student.S_ID = '$student_id' AND sick_leave.Si_ID = '$sick_leave_id' AND sick_leave.A_ID = '$advisor_id'";
        $result_sick_leave_info = $conn->query($sql_sick_leave_info);

        if ($result_sick_leave_info->num_rows > 0) {
            $row_sick_leave_info = $result_sick_leave_info->fetch_assoc();
            $student_name = $row_sick_leave_info['student_name'];
            $upload_date = $row_sick_leave_info['Date'];
            $current_status = $row_sick_leave_info['Status'];
            $file_path = $row_sick_leave_info['file_path'];
        } else {
            echo "Sick leave not found or not assigned to the advisor.";
            exit();
        }
    } else {
        echo "Both Student ID (S_ID) and Sick Leave ID (Si_ID) must be provided.";
        exit();
    }
} else {
...
<li>
<a href="Adv_Sickleaves.php" class="nav-link link-body-emphasis" style="background-color:#f3f2f5;">
...
<div class="card  w-90 d-block" style="margin: 60px 20px 0px 20px ; height : 450px;" >
  ...
  <div class="card-body" >
 <h5>Modify Sick Leave Status for <b><?php echo $student_name; ?></b></h5>
 <br>
    ...
    <div class="col-sm-10">
        <select class="form-control" id="exampleFormControlSelect1" name="new_status" required>
